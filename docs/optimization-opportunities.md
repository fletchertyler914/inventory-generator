...
### 4. Notes Query - File Counts ✅ COMPLETED

**Location**: `src-tauri/src/lib.rs:963`

**Implementation**:

- Backend uses GROUP BY query: `SELECT file_id, COUNT(*) as count FROM notes WHERE case_id = ? AND file_id IS NOT NULL GROUP BY file_id`
- Returns only file_id and count (much smaller payload)
- Frontend receives direct map of file_id -> count

**Performance Impact**:

- ✅ Smaller payload (10x smaller for 10k notes)
- ✅ Faster processing (SQL does counting, not JS)
- ✅ Already optimized with proper SQL aggregation
...
4. ✅ **Notes count GROUP BY** - DONE (uses GROUP BY in SQL)
